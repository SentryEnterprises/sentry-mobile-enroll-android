...
//----------------------------------------------------------------------------------------------------------------------
int lib_enroll_check_sw(uint8_t* apdu_enroll, uint32_t  apdu_enroll_len)
{
	uint16_t SW;
	int resp = 0;
	SW = lib_get_sw(apdu_enroll, apdu_enroll_len);
	switch (SW)
	{
		//------------
	case	0x9000:
...
int lib_enroll_select(void)
{
	...
	Ret = apdu_secure_channel("== Enroll Select", apdu_select, sizeof(apdu_select), apdu_enroll_out, &apdu_enroll_out_len);
...
int lib_enroll_verify_pin(uint8_t* pin, int len)
{
	...
	if (len < 4) return _SDK_ERROR_PIN_UNDERSIZED_;//-1;
	if (len > 8) return _SDK_ERROR_PIN_OVERSIZED_;//-1;
	...
	apdu_enroll_out_len = 0;
	Ret = apdu_secure_channel("== Enroll Verify Pin", apdu_verify_pin, sizeof(apdu_verify_pin), apdu_enroll_out, &apdu_enroll_out_len);
	if (Ret != 0) return Ret;
    ...
    // if apdu_secure_channel didn't get a 0x9000, we should already be returning the SW
...
int lib_enroll_status(uint8_t* max_num_fingers, uint8_t* enrolled_touches, uint8_t* remaining_touches, uint8_t* biometric_mode )
{
	...
	do
	{
		uint8_t apdu_status[] = { 0x80, 0x59, 0x04, 0x00, 0x01, 0x00 };
		apdu_enroll_out_len = 0;
		Ret = apdu_secure_channel("== Enroll Status", apdu_status, sizeof(apdu_status), apdu_enroll_out, &apdu_enroll_out_len);
...
int lib_enroll_reprocess(int num_finger, uint8_t* enrolled_touches, uint8_t* remaining_touches, uint8_t* biometric_mode)
{
	...
	do
	{
		uint8_t apdu_process[] = { 0x80, 0x59, 0x03, 0x00, 0x02, 0x02, 0x01 };
		apdu_enroll_out_len = 0;
		Ret = apdu_secure_channel("== Enroll Reprocess", apdu_process, sizeof(apdu_process), apdu_enroll_out, &apdu_enroll_out_len);
		if (Ret != 0) return Ret;
		Ret = lib_enroll_check_sw(apdu_enroll_out, apdu_enroll_out_len);
		if (Ret == 0) break;
		if (Ret == 1) continue;
