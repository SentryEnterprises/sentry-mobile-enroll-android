...
//----------------------------------------------------------------------------------------------------------------------
int apdu_secure_channel(const char* caller, uint8_t* DataIn, uint32_t DataInLen, uint8_t* DataOut, uint32_t* DataOutLen)
{
...
//	}
    ...
    syslog(LOG_CRIT, "Secure Channel (%s)", caller);

	if (ApduIsSecureChannel == 0)
	{
        syslog(LOG_CRIT, "Secure Channel - Raw Call");
		Ret = pSmartCardApduCallBack(DataIn, DataInLen, DataOut, DataOutLen);
        if (Ret != _SDK_SUCCESS_) return Ret;
	}
	else
	{
        syslog(LOG_CRIT, "Secure Channel - Secure Call");
        ...
        if ((DataIn[0] & 0xFF) != 0x84)
		{
            syslog(LOG_CRIT, "Secure Channel - Not really, raw call");
			Ret = pSmartCardApduCallBack(DataIn, DataInLen, DataOut, DataOutLen);
            if (Ret != _SDK_SUCCESS_) return Ret;
		}
        else {
            syslog(LOG_CRIT, "Secure Channel - Wrapping");
            Ret = lib_auth_wrap(DataIn, DataInLen, wrap_apdu_out, &wrap_out_len);
            if (Ret != _SDK_SUCCESS_) return Ret;

            syslog(LOG_CRIT, "Secure Channel - Actual Call");
            Ret = pSmartCardApduCallBack(wrap_apdu_out, wrap_out_len, unwrap_apdu_out, &unwrap_out_len);
            if (Ret != _SDK_SUCCESS_) return Ret;

            if (unwrap_out_len != 2) {
                syslog(LOG_CRIT, "Secure Channel - Unwrapping");
                Ret = lib_auth_unwrap(unwrap_apdu_out, unwrap_out_len, DataOut, DataOutLen);
                if (Ret != _SDK_SUCCESS_) return Ret;
            } else {
                syslog(LOG_CRIT, "Secure Channel - No need to unwrap");
    ...
    syslog(LOG_CRIT, "Secure Channel - Status word ok, returning");
	return Ret;
}
...
int unwrap(uint8_t* apdu_in, uint32_t in_len, uint8_t* apdu_out, uint32_t* out_len, uint8_t* key_enc, uint8_t* key_rmac)
{
	...
	if (in_len < 10) return _SDK_ERROR_UNWRAP_LENGTH_; //-1; //not RMAC
	...
	if (memcmp(tmp_chaining_value, apdu_in + lcmac, 8) != 0) return _SDK_ERROR_UNWRAP_DECRYPT_; //-3;
	...
	if (in_len > 16 + 8 + 1)
	{
		...
		lcenc = in_len - 10;
		if ((lcenc % 16) > 0) return _SDK_ERROR_UNWRAP_ALIGN_;// -2;
		...
		for (p = lcenc - 1; p > 0; p--)
		{
			...
			return _SDK_ERROR_UNWRAP_DECRYPT_; //-3;
		}
