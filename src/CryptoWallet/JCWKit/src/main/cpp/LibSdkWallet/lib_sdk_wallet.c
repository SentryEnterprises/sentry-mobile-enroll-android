...
{
    syslog(LOG_CRIT, "-- Secure Channel Init --");
	...
	apdu_out_len = 0;
	ret = apdu_secure_channel("== Secure Channel Init", apdu_int, len_apdu, apdu_out, &apdu_out_len);
...
{
	uint16_t sw;
	if (len < 2) return _SDK_ERROR_APDU_RESPONSE_UNDERSIZED_;//0;
	sw = (pout[len - 2] << 8) + (pout[len - 1] << 0);	
	return sw;
}
//----------------------------------------------------------------------------------------------------------------------
...
{
    syslog(LOG_CRIT, "-- Wallet Select --");
	uint8_t apdu_select[] = { 0x00, 0xA4, 0x04, 0x00, 0x0A, 0x4A, 0x4E, 0x45, 0x54, 0x5F, 0x4C, 0x5F, 0x01, 0x01, 0x57, 0x00 };
	apdu_out_len = 0;
	int Ret = apdu_secure_channel("== Select Wallet", apdu_select, sizeof(apdu_select), apdu_out, &apdu_out_len);
	if (Ret != 0) return Ret;// _SDK_ERROR_EXCHANGE_;
...
{
    syslog(LOG_CRIT, "-- Wallet Select SC --");
	uint8_t apdu_select[] = { 0x80, 0xA4, 0x04, 0x00, 0x0A, 0x4A, 0x4E, 0x45, 0x54, 0x5F, 0x4C, 0x5F, 0x01, 0x01, 0x57, 0x00 };
	apdu_out_len = 0;
	int Ret = apdu_secure_channel("== Select Wallet SC", apdu_select, sizeof(apdu_select), apdu_out, &apdu_out_len);
	if (Ret != 0) return Ret;// _SDK_ERROR_EXCHANGE_;
...
int lib_wallet_get_version(uint8_t *version)
{
	...
	if (Ret != 0) return Ret;
	Ret = apdu_secure_channel("== Wallet Get Version", apdu_get_data, sizeof(apdu_get_data), apdu_out, &apdu_out_len);
...
int lib_os_get_version(uint8_t* version)
{
	...
	apdu_out_len = 0;	
	int Ret = apdu_secure_channel("== OS Get Version", apdu_get_data, sizeof(apdu_get_data), apdu_out, &apdu_out_len);
	if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
	Ret = lib_check_sw_err(apdu_out, apdu_out_len);
	if (Ret != 0) return Ret;
...
int lib_wallet_get_gguid(uint8_t* gguid)
{
	...
	apdu_out_len = 0;
	int Ret = apdu_secure_channel("== Wallet Get GGUID", apdu_get_data, sizeof(apdu_get_data), apdu_out, &apdu_out_len);
	if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
	Ret = lib_check_sw_err(apdu_out, apdu_out_len);
	if (Ret != 0) return Ret;
	if (Ret == 0 )//5F 40 20
...
int lib_sdk_get_capability(uint8_t* capability)
{
	...
	apdu_out_len = 0;	
	int Ret = apdu_secure_channel("== Get Capability", apdu_get_cap, sizeof(apdu_get_cap), apdu_out, &apdu_out_len);
	if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
	Ret = lib_check_sw_err(apdu_out, apdu_out_len);
	if (Ret != 0) return Ret;
...
int lib_wallet_get_status(uint8_t *GWLC, uint8_t* WPSM, uint8_t* WSSM)
{
	...
	//if (lib_wallet_select_wallet_sc() != 0) return -1;
	apdu_out_len = 0;
	int Ret = apdu_secure_channel("== Wallet Get Status", apdu_get_data, sizeof(apdu_get_data), apdu_out, &apdu_out_len);
	if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
	Ret = lib_check_sw_err(apdu_out, apdu_out_len);
	if (Ret != 0) return Ret;
...
int lib_wallet_get_status_account(uint8_t* ALC, uint8_t* WSSM)
{
	...
	apdu_out_len = 0;
	int Ret = apdu_secure_channel("== Wallet Get Status Account", apdu_get_data, sizeof(apdu_get_data), apdu_out, &apdu_out_len);
	if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
	Ret = lib_check_sw_err(apdu_out, apdu_out_len);
	if (Ret != 0) return Ret;
...
int lib_wallet_get_accounts(uint8_t *NumberAccounts, uint8_t *AccountInfo)
{
	...
	if (Ret != 0) return Ret;
	Ret = apdu_secure_channel("== Get Accounts", apdu_get_data, sizeof(apdu_get_data), apdu_out, &apdu_out_len);
	if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
	Ret = lib_check_sw_err(apdu_out, apdu_out_len);
	if (Ret != 0) return Ret;
	...
	if (Ret == 0 )//BF C4 04 9F 30 01 00 90 00  or
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			Ret = apdu_secure_channel("== Get Account", apdu_get_info, sizeof(apdu_get_info), apdu_out, &apdu_out_len);
			if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
			Ret = lib_check_sw_err(apdu_out, apdu_out_len);
			if (Ret != 0) return Ret;
			p = 0;
...
int lib_wallet_store_pin(uint8_t* pin, int len)
{
	...
	apdu_out_len = 0;
	Ret = apdu_secure_channel("== Wallet Store PIN", apdu_store_pin, sizeof(apdu_store_pin), apdu_out, &apdu_out_len);
	if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
	Ret = lib_check_sw_err(apdu_out, apdu_out_len);
	if (Ret != 0) return Ret;
...
{
    syslog(LOG_CRIT, "-- Wallet Verify PIN --");
	int p = 5,Ret=0;
	uint8_t b;
    ...
    syslog(LOG_CRIT, "-- Wallet Verify PIN: Selecting Wallet --");
	if (SelectWallet != 1) Ret=lib_wallet_select_wallet_sc();
	if (Ret != 0) return Ret;

	apdu_out_len = 0;
	Ret = apdu_secure_channel("== Wallet Verify PIN", apdu_verify_pin, sizeof(apdu_verify_pin), apdu_out, &apdu_out_len);
	if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
...
{	
	...
	apdu_out_len = 0;
	len_mnemonic[0] = 0;
	int Ret = apdu_secure_channel("== Wallet Create", apdu_create, sizeof(apdu_create), apdu_out, &apdu_out_len);
	if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
	Ret = lib_check_sw_err(apdu_out, apdu_out_len);
	if (Ret != 0) return Ret;
...
int lib_wallet_recovery(int iteration, uint8_t* mnemonics, int len_mnemonics, uint8_t* passphrase, int len_passphrase)
{
	...
	for (i = 0; i < block_240; i++)
	{
		...
		apdu_out_len = 0;
		Ret = apdu_secure_channel("== Wallet Recovery", apdu_init_recovery, len_send, apdu_out, &apdu_out_len);
		if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
		Ret = lib_check_sw_err(apdu_out, apdu_out_len);
		if (Ret != 0) return Ret;
	...
	
	{
		...
		apdu_out_len = 0;
		Ret = apdu_secure_channel("== Wallet Recovery 2", apdu_init_recovery, len_send, apdu_out, &apdu_out_len);
		if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
		Ret = lib_check_sw_err(apdu_out, apdu_out_len);
		if (Ret != 0) return Ret;
	...
	apdu_out_len = 0;
	Ret = apdu_secure_channel("== Wallet Recovery 3", apdu_recovery, len_send, apdu_out, &apdu_out_len);
	if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
	Ret = lib_check_sw_err(apdu_out, apdu_out_len);
	if (Ret != 0) return Ret;
...
int lib_wallet_account_create(uint32_t currID, uint8_t netID, uint8_t accountID, uint8_t chain, uint8_t bip, uint8_t *nickname, int len_nick  )
{
	...
	apdu_out_len = 0;
	Ret = apdu_secure_channel("== Wallet account create", apdu_account, len_send, apdu_out, &apdu_out_len);
	if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
	Ret = lib_check_sw_err(apdu_out, apdu_out_len);
	if (Ret != 0) return Ret;
...
{
    syslog(LOG_CRIT, "-- Wallet CVM Status --");
	int p = 0;
	uint8_t apdu_get_data[] = { 0x80, 0xB6, 0x00, 0x00, 0x00 };
	apdu_out_len = 0;

	int Ret = apdu_secure_channel("== Wallet Get CVM Status", apdu_get_data, sizeof(apdu_get_data), apdu_out, &apdu_out_len);
	if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
	Ret = lib_check_sw_err(apdu_out, apdu_out_len);
	if (Ret != 0) return Ret;
...
{
    syslog(LOG_CRIT, "-- Wallet CVM Verify --");
	int p = 0;
	uint8_t apdu_get_data[] = { 0x80, 0xB6, 0x01, 0x00, 0x00 };
	apdu_out_len = 0;

	int Ret = apdu_secure_channel("== Wallet Get CVM Verify", apdu_get_data, sizeof(apdu_get_data), apdu_out, &apdu_out_len);
	if (Ret != _SDK_SUCCESS_) return Ret;// _SDK_ERROR_EXCHANGE_;
...
//	if (Ret != 0) return Ret;
	...
	//{
		int len_tag;
		if (apdu_out[p++] != 0x5F) return _SDK_ERROR_CVM_VERIFY_;
		if (apdu_out[p++] != 0x3C) return _SDK_ERROR_CVM_VERIFY_;
		len_tag = apdu_out[p++];
		if (len_tag != 2) return _SDK_ERROR_CVM_VERIFY_;
		memcpy(CVM, apdu_out + p, len_tag);
		p += len_tag;

		if (apdu_out[p++] != 0x5F) return _SDK_ERROR_CVM_VERIFY_;
		if (apdu_out[p++] != 0x39) return _SDK_ERROR_CVM_VERIFY_;
		len_tag = apdu_out[p++];
		if (len_tag != 1) return _SDK_ERROR_CVM_VERIFY_;
		memcpy(WSSM, apdu_out + p, len_tag);
	//}
	return Ret;
}
...
int lib_wallet_cvm_disable_pin(void)
{
	...
	apdu_out_len = 0;
	int Ret = apdu_secure_channel("== Disable PIN", apdu_get_data, sizeof(apdu_get_data), apdu_out, &apdu_out_len);
	if (Ret != 0) return _SDK_ERROR_EXCHANGE_;
	Ret = lib_check_sw_err(apdu_out, apdu_out_len);
	if (Ret != 0) return Ret;
